...
def predict(model_name, img):
    ...
    if model_name == "My_convolution_layer_model":
        new_model = load_model(
            'C:/Users/16189/Documents/hyperspectral/retina_augmented_model__new_mymodel.h5'
        )
        ...
        new_model = load_model(
            'C:/Users/16189/Documents/hyperspectral/retina_augmented_model__new_mymodel_transfer.h5'
        )
...
def prediction(model, image, patch_size):
    ...
    for i in range(0, image.shape[0], patch_size):
        for j in range(0, image.shape[1], patch_size):
            ...
            single_patch_norm = np.expand_dims(
                normalize(np.array(single_patch), axis=1), 2
            )
            single_patch_shape = single_patch_norm.shape[:2]
            single_patch_input = np.expand_dims(single_patch_norm, 0)
            single_patch_prediction = (
                model.predict(single_patch_input)[0, :, :, 0] > 0.5
            ).astype(np.uint8)
            segm_img[
                i : i + single_patch_shape[0], j : j + single_patch_shape[1]
            ] += cv2.resize(single_patch_prediction, single_patch_shape[::-1])
...
def get_image(model_name, name):
    if name == "Diabetic retinopathy classification":
        image = st.sidebar.file_uploader(
            label="Select an retinal fundus image", type=["jpg", "jpeg", "png"]
        )
    ...
    if button:
            ...
            with open(os.path.join(new_path, "segmented_" + image.name), "wb") as f:
